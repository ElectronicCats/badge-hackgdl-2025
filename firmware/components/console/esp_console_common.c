...
esp_err_t esp_console_setup_prompt(const char *prompt,
...
esp_err_t esp_console_setup_history(const char *history_path,
  ...
  if (linenoiseHistorySetMaxLen(max_history_len) != 1) {
    ESP_LOGE(TAG, "set max history length to %" PRIu32 " failed",
             max_history_len);
    ret = ESP_FAIL;
    goto _exit;
  }
...
esp_err_t esp_console_common_init(size_t max_cmdline_length,
                                  esp_console_repl_com_t *repl_com) {
  esp_err_t ret = ESP_OK;
...
void esp_console_repl_task(void *args) {
  esp_console_repl_universal_t *repl_conf =
      (esp_console_repl_universal_t *)args;
