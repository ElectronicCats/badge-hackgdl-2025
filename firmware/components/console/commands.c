...
typedef struct cmd_item_ {
  ...
  esp_console_cmd_func_with_context_t
      func_w_context; //!< pointer to the command handler (with user context)
...
esp_err_t esp_console_init(const esp_console_config_t *config) {
  ...
  s_tmp_line_buf =
      heap_caps_calloc(1, config->max_cmdline_length, s_config.heap_alloc_caps);
...
esp_err_t esp_console_cmd_register(const esp_console_cmd_t *cmd) {
  ...
  if (!item) {
    ...
    // remove from list and free the old hint, because we will alloc new hint
    // for the command
...
const char *esp_console_get_hint(const char *buf, int *color, int *bold) {
  ...
  cmd_item_t *it;
  SLIST_FOREACH(it, &s_cmd_list, next) {
    if (strlen(it->command) == len && strncmp(buf, it->command, len) == 0) {
...
esp_err_t esp_console_run(const char *cmdline, int *cmd_ret) {
  ...
  char **argv = (char **)heap_caps_calloc(
      s_config.max_cmdline_args, sizeof(char *), s_config.heap_alloc_caps);
  if (argv == NULL) {
    return ESP_ERR_NO_MEM;
  }
  ...
  size_t argc =
      esp_console_split_argv(s_tmp_line_buf, argv, s_config.max_cmdline_args);
  if (argc == 0) {
    free(argv);
    return ESP_ERR_INVALID_ARG;
  }
...
static int help_command(int argc, char **argv) {
  ...
  if (help_args.help_cmd->count == 0) {
    ...
    printf("Recommended terminal emulator apps: Minicom (Linux/macOS), PuTTY "
           "(Windows)\n"
    ...
    if (!found_command) {
      printf("help: Unrecognized option '%s'. Please use correct command as "
             "argument "
