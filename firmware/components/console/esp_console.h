...
typedef struct {
  ...
  size_t
      max_cmdline_args; //!< maximum number of command line arguments to parse
  uint32_t heap_alloc_caps; //!< where to (e.g. MALLOC_CAP_SPIRAM) allocate
                            //!< heap objects such as cmds used by esp_console
...
typedef struct {
  ...
  const char *history_save_path; //!< file path used to save history commands,
                                 //!< set to NULL won't save to file system
  uint32_t task_stack_size;      //!< repl task stack size
  uint32_t task_priority;        //!< repl task priority
  BaseType_t task_core_id; //!< repl task affinity, i.e. which core the task is
                           //!< pinned to
  const char *prompt;      //!< prompt (NULL represents default: "esp> ")
  size_t max_cmdline_length; //!< maximum length of a command line. If 0,
 ...
 * @param argv array with argc entries, each pointing to a zero-terminated
 * string argument
 ...
 * @return console command return code, 0 indicates "success"
 */
typedef int (*esp_console_cmd_func_with_context_t)(void *context, int argc,
                                                   char **argv);
 ...
 *
 * @note If the member \c func_w_context of cmd is set instead of func, then the
 * member \c context is passed to the function pointed to by \c func_w_context.
 *
 * @return
 *      - ESP_OK on success
 *      - ESP_ERR_NO_MEM if out of memory
 ...
 * @param[out] ret_repl return REPL handle after initialization succeed, return
 * NULL otherwise
 *
 * @note This is an all-in-one function to establish the environment needed for
 * REPL, includes:
 *       - Install the UART driver on the console UART (8n1, 115200, REF_TICK
 * clock source)
 ...
 *
 * @attention This function is meant to be used in the examples to make the code
 * more compact. Applications which use console functionality should be based on
 ...
 * NULL otherwise
 *
 * @note This is a all-in-one function to establish the environment needed for
 * REPL, includes:
 *       - Initializes linenoise
 *       - Spawn new thread to run REPL in the background
 *
...
esp_console_new_repl_usb_cdc(const esp_console_dev_usb_cdc_config_t *dev_config,
    ...
    (defined __DOXYGEN__ && SOC_USB_SERIAL_JTAG_SUPPORTED)
/**
 * @brief Establish a console REPL (Read-eval-print loop) environment over
 * USB-SERIAL-JTAG
 *
 ...
 * @param[in] repl REPL handle returned from esp_console_new_repl_xxx
 * @note Once the REPL gets started, it won't be stopped until the user calls
 * repl->del(repl) to destroy the REPL environment.
 * @return
 *      - ESP_OK on success
