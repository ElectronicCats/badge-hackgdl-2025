...
static esp_err_t
esp_console_repl_usb_serial_jtag_delete(esp_console_repl_t *repl);
#endif // CONFIG_ESP_CONSOLE_USB_SERIAL_JTAG

#if CONFIG_ESP_CONSOLE_USB_CDC
esp_err_t
esp_console_new_repl_usb_cdc(const esp_console_dev_usb_cdc_config_t *dev_config,
  ...
  // setup history
  ret = esp_console_setup_history(repl_config->history_save_path,
                                  repl_config->max_history_len,
                                  &cdc_repl->repl_com);
  if (ret != ESP_OK) {
    goto _exit;
  }
  ...
  if (xTaskCreatePinnedToCore(
          esp_console_repl_task, "console_repl", repl_config->task_stack_size,
          cdc_repl, repl_config->task_priority, &cdc_repl->repl_com.task_hdl,
          repl_config->task_core_id) != pdTRUE) {
    ret = ESP_FAIL;
    goto _exit;
  }
...
esp_err_t esp_console_new_repl_usb_serial_jtag(
  ...
  // setup prompt
  esp_console_setup_prompt(repl_config->prompt,
                           &usb_serial_jtag_repl->repl_com);

  /* Fill the structure here as it will be used directly by the created task. */
  usb_serial_jtag_repl->uart_channel = CONFIG_ESP_CONSOLE_UART_NUM;
  usb_serial_jtag_repl->repl_com.state = CONSOLE_REPL_STATE_INIT;
  usb_serial_jtag_repl->repl_com.repl_core.del =
      esp_console_repl_usb_serial_jtag_delete;

  /* spawn a single thread to run REPL */
  if (xTaskCreatePinnedToCore(esp_console_repl_task, "console_repl",
                              repl_config->task_stack_size,
                              usb_serial_jtag_repl, repl_config->task_priority,
                              &usb_serial_jtag_repl->repl_com.task_hdl,
                              repl_config->task_core_id) != pdTRUE) {
    ret = ESP_FAIL;
    goto _exit;
  }
...
#if CONFIG_ESP_CONSOLE_UART_DEFAULT || CONFIG_ESP_CONSOLE_UART_CUSTOM
esp_err_t
esp_console_new_repl_uart(const esp_console_dev_uart_config_t *dev_config,
                          const esp_console_repl_config_t *repl_config,
                          esp_console_repl_t **ret_repl) {
  esp_err_t ret = ESP_OK;
  ...
  /* Minicom, screen, idf_monitor send CR when ENTER key is pressed */
  uart_vfs_dev_port_set_rx_line_endings(dev_config->channel,
                                        ESP_LINE_ENDINGS_CR);
  /* Move the caret to the beginning of the next line on '\n' */
  uart_vfs_dev_port_set_tx_line_endings(dev_config->channel,
                                        ESP_LINE_ENDINGS_CRLF);

/* Configure UART. Note that REF_TICK/XTAL is used so that the baud rate
 * remains correct while APB frequency is changing in light sleep mode.
 */
...
#error "No UART clock source is aware of DFS"
  ...
  uart_set_pin(dev_config->channel, dev_config->tx_gpio_num,
               dev_config->rx_gpio_num, -1, -1);
  ...
  if (xTaskCreatePinnedToCore(
          esp_console_repl_task, "console_repl", repl_config->task_stack_size,
          uart_repl, repl_config->task_priority, &uart_repl->repl_com.task_hdl,
          repl_config->task_core_id) != pdTRUE) {
    ret = ESP_FAIL;
    goto _exit;
  }
...
static esp_err_t esp_console_repl_uart_delete(esp_console_repl_t *repl) {
  ...
  esp_console_repl_com_t *repl_com =
      __containerof(repl, esp_console_repl_com_t, repl_core);
  esp_console_repl_universal_t *uart_repl =
      __containerof(repl_com, esp_console_repl_universal_t, repl_com);
...
static esp_err_t esp_console_repl_usb_cdc_delete(esp_console_repl_t *repl) {
  ...
  esp_console_repl_com_t *repl_com =
      __containerof(repl, esp_console_repl_com_t, repl_core);
  esp_console_repl_universal_t *cdc_repl =
      __containerof(repl_com, esp_console_repl_universal_t, repl_com);
  // check if already de-initialized
  if (repl_com->state == CONSOLE_REPL_STATE_DEINIT) {
    ESP_LOGE(TAG, "already de-initialized");
...
esp_console_repl_usb_serial_jtag_delete(esp_console_repl_t *repl) {
  ...
  esp_console_repl_com_t *repl_com =
      __containerof(repl, esp_console_repl_com_t, repl_core);
  esp_console_repl_universal_t *usb_serial_jtag_repl =
      __containerof(repl_com, esp_console_repl_universal_t, repl_com);
  // check if already de-initialized
  if (repl_com->state == CONSOLE_REPL_STATE_DEINIT) {
    ESP_LOGE(TAG, "already de-initialized");
