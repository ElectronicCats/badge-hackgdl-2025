...
/*
 * NOTE: Most of these unit tests DO NOT work standalone. They require pytest to
 * control the application and check for correct output. E.g., to run the test
 * "esp console help command - reverse registration", type: pytest --target
 * esp32 -m "generic" -k test_console_help_reverse_registration The pytest test
 * cases are different than the unit test cases here, they can be found in the
 * pytest_*.py file in the root directory of this test project. For more
 * information on pytest, please refer to
...
static int do_not_call(void *context, int argc, char **argv) {
  TEST_ASSERT_MESSAGE(false,
                      "This function is a dummy and must not be called\n");
  return 0;
}

TEST_CASE(
    "esp console register with normal and context aware functions set fails",
    "[console]") {
...
TEST_CASE(
    "esp console register with normal and context aware function set to NULL "
    "fails",
    "[console]") {
  esp_console_config_t console_config = ESP_CONSOLE_CONFIG_DEFAULT();
  TEST_ESP_OK(esp_console_init(&console_config));
...
TEST_CASE("esp console init function NULL param fails", "[console]") {
  ...
  TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG,
                    esp_console_new_repl_uart(NULL, &repl_config, &s_repl));
  TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG,
                    esp_console_new_repl_uart(&uart_config, NULL, &s_repl));
  TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, esp_console_new_repl_uart(
                                             &uart_config, &repl_config, NULL));
}
...
static esp_console_cmd_t s_quit_cmd = {
    .command = "quit", .help = "Quit REPL environment", .func = &do_cmd_quit};
...
TEST_CASE("esp console help command - sorted registration",
          "[console][ignore]") {
  esp_console_repl_config_t repl_config = ESP_CONSOLE_REPL_CONFIG_DEFAULT();
  esp_console_dev_uart_config_t uart_config =
      ESP_CONSOLE_DEV_UART_CONFIG_DEFAULT();
 ...
 */
TEST_CASE("esp console help command - reverse registration",
          "[console][ignore]") {
  esp_console_repl_config_t repl_config = ESP_CONSOLE_REPL_CONFIG_DEFAULT();
  esp_console_dev_uart_config_t uart_config =
      ESP_CONSOLE_DEV_UART_CONFIG_DEFAULT();
