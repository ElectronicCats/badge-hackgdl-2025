...
/**
 * Depending on if the input is a terminal or a file or pipe, we need to apply
 * different settings to avoid additional processing or buffering getting into
 * our way.
 */
static void prepare_input_stream(void) {
  ...
  if (isatty(stdin_fileno)) {
    // Use Termios driver to activate CR-NL translation and deactivate echo and
    // canonical mode
...
static esp_err_t esp_console_repl_linux_delete(esp_console_repl_t *repl) {
  ...
  esp_console_repl_universal_t *linux_repl =
...
static esp_err_t
esp_console_new_repl_linux(const esp_console_repl_config_t *repl_config,
  ...
  // Make sure the setup works on Linux without buffering or additional
  // processing
  ...
  if (xTaskCreate(esp_console_repl_task, "console_repl",
                  repl_config->task_stack_size, linux_repl,
                  repl_config->task_priority,
                  &linux_repl->repl_com.task_hdl) != pdTRUE) {
    ret = ESP_FAIL;
    goto _exit;
  }
