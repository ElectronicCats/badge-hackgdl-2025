...
{
  if (mask == MASK_6B || mask == 0xFCF)
    return 4;
  if (code == 0xA80 || code == 0xA90 || code == 0xAA0 || code == 0xAB0 || code == 0xAC0 || code == 0xAD0 || code == 0xAE0 || code == 0xEC0 || code == 0xEE0 || code == 0xEF0 || code == 0xF00 || code == 0xF10)
    return 2;
  return 0;
}
...
int cpu_step(void)
{
  ...
  // Verificar si el código de operación es desconocido
  if (!ops0[i].cycles)
  {
    // ESP_LOGE("CPU", "Unknown op-code 0x%X (pc = 0x%04X)", op, pc);
    return 1;
  }

  // Almacenar los valores de la operación
  op_t0 ops;
  ...
  // Mostrar el estado actual del procesador (implementación de impresión no proporcionada)
  print_state(i, op, pc);

  // Sincronización del procesador con el tiempo
  ref_ts = wait_for_cycles(ref_ts, previous_cycles);

  op_t1 ops11;
  ops11.cb1 = ops1[i].cb1;

  // Procesar el código de operación
  if (ops11.cb1 != NULL)
  {
  ...
  // Preparar para la siguiente instrucción
  pc = next_pc;
