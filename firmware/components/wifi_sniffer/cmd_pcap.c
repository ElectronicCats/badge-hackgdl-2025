...
void wifi_sniffer_register_summary_cb(summary_cb_t cb) { summary_cb = cb; }
...
static esp_err_t pcap_open(pcap_cmd_runtime_t *pcap) {
  ...
  //   files_ops_incremental_name(pcap_dir, "analizer", ".pcap",
  //   pcap->filename); free(pcap_dir); fp = fopen(pcap->filename, "wb+");
...
esp_err_t packet_capture(void *payload, uint32_t length, uint32_t seconds,
