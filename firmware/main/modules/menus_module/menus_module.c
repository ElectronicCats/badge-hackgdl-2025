...
static app_state2_t app_state = {
    .in_app = false, .input_callback = NULL, .input_last_callback = NULL};
...
static void display_menus() { menus_screens_display_menus(menus_ctx); }
...
void menus_module_enable_input() { menus_ctx->input_lock = false; }
void menus_module_disable_input() { menus_ctx->input_lock = true; }
...
menu_idx_t menus_module_get_current_menu() { return menus_ctx->current_menu; }

bool menus_module_get_app_state() { return app_state.in_app; }
...
void menus_module_refresh() { refresh_menus(); }

void menus_module_begin() {
  menus_ctx = calloc(1, sizeof(menus_manager_t));
  menus_ctx->menus_count = get_menus_count();
  // screen_saver_begin();
  // sleep_mode_begin();
  keyboard_module_set_input_callback(menus_input_cb);
