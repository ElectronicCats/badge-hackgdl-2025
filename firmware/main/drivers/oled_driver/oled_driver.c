...
int oled_driver_get_width(oled_driver_t *dev) { return dev->_width; }

int oled_driver_get_height(oled_driver_t *dev) { return dev->_height; }

int oled_driver_get_pages(oled_driver_t *dev) { return dev->_pages; }
...
void oled_driver_display_image(oled_driver_t *dev, int page, int seg,
...
void oled_driver_display_text(oled_driver_t *dev, int page, char *text, int x,
                              bool invert) {
  if (page >= dev->_pages)
    return;
  ...
  for (uint8_t i = 0; i < _text_len; i++) {
    memcpy(image, font8x8_basic_tr[(uint8_t)text[i] + encrypt] + typography, 8);
    if (invert)
...
// by Coert Vonk
void oled_driver_display_text_x3(oled_driver_t *dev, int page, char *text,
...
void oled_driver_scroll_text(oled_driver_t *dev, char *text, int text_len,
                             bool invert) {
...
void oled_driver_wrap_arround(oled_driver_t *dev,
                              oled_driver_scroll_type_t scroll, int start,
                              int end, int8_t delay) {
...
void oled_driver_bitmaps(oled_driver_t *dev, int xpos, int ypos,
                         uint8_t *bitmap, int width, int height, bool invert) {
  if ((width % 8) != 0) {
...
void oled_driver_draw_pixel(oled_driver_t *dev, int xpos, int ypos,
                            bool invert) {
...
void _oled_driver_line(oled_driver_t *dev, int x1, int y1, int x2, int y2,
                       bool invert) {
  int i;
  int dx, dy;
  int sx, sy;
...
uint8_t oled_driver_copy_bit(uint8_t src, int srcBits, uint8_t dst,
                             int dstBits) {
...
void oled_driver_draw_modal_box(oled_driver_t *dev, int pos_x,
                                int modal_height) {
...
void oled_driver_set_encrypt_value(bool value) { encrypt = value; }

void oled_driver_set_typography_value(bool value) { typography = value; }