...
static void deauth_increment_item() { current_item++; }

static void deauth_decrement_item() { current_item--; }
...
static void deauth_module_cb_event_captive_portal(uint8_t button_name,
  ...
  switch (button_name) {
  case BUTTON_MIDDLE:
    current_item = current_item-- == 0 ? CAPTIVEPORTALCOUNT - 1 : current_item;
    deauth_display_captive_portals(current_item, menu_stadistics);
    break;
  case BUTTON_BACK:
