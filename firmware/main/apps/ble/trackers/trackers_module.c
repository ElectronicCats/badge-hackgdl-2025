...
void trakers_module_traker_details(uint8_t selection) {
  char tracker_mac[18];
  snprintf(tracker_mac, sizeof(tracker_mac), "%02X:%02X:%02X:%02X:%02X",
           scanned_airtags[selection].mac_address[1],
           scanned_airtags[selection].mac_address[2],
           scanned_airtags[selection].mac_address[3],
           scanned_airtags[selection].mac_address[4],
           scanned_airtags[selection].mac_address[5]);
  char tracker_rssi[18];
  snprintf(tracker_rssi, sizeof(tracker_rssi), "RSSI: %d",
           scanned_airtags[selection].rssi);
  char tracker_name[18];
  snprintf(tracker_name, sizeof(tracker_name), "Name: %s",
           scanned_airtags[selection].name);
...
void trakers_module_scan_exit() { menus_module_restart(); }
void trakers_module_list_exit() { trakers_scenes_main_menu(); }
